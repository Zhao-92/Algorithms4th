...
这里使用`BeanDefinition`来封装了bean对象，这样可以保存一些额外的元信息。BeanFactory存放了所有的Bean元素在Map中。注入的过程就是将这个bean所对应对象的放入bean工厂中

BeanFactory

- Map<String, BeanDefinition> 
- getBean(String)
- registerBeanDefinition(String, BeanDefiniton)
...
在`BeanDefinition`中有个Class类的对象beanClass，创建bean时将这个类的全限定名传入，再通过Class.forName( )方法生成这个类的Class对象。在`AutowireCapableBeanFactory`中通过Class类的newInstance( )方法创建这个bean的实例
...
现在BeanFactory的功能齐全了(读取配置文件，初始化BeanFactory，注册bean，注入bean属性，初始化bean)，但是使用起来有点麻烦。于是我们引入熟悉的`ApplicationContext`接口，并在`AbstractApplicationContext`的`refresh()`方法中进行bean的初始化工作。
...
在Spring AOP中，我觉得最重要的两个角色，就是我们熟悉的`MethodInterceptor`和`MethodInvocation`（这两个角色都是AOP联盟的标准），它们分别对应AOP中两个基本角色：`Advice`和`Joinpoint`。**Advice定义了在切点指定的逻辑**，而**Joinpoint则代表切点**。
